...
async def main():
    ...
    READABLE_JSON_PATH=os.getenv("READABLE_JSON_PATH","out/facts_pretty.json")
    ...
    if DRY_RUN:
        ...
        # Дополнительно — читаемые файлы: Markdown + JSON
        try:
            ...
            # JSON (pretty)
            os.makedirs(os.path.dirname(READABLE_JSON_PATH) or ".", exist_ok=True)
            readable_entries = []
            for r in results:
                p = r.get("persona", {}) or {}
                entry = {
                    "chunk_index": r.get("chunk_index"),
                    "id_range": [r.get("chunk_first_id"), r.get("chunk_last_id")],
                    "summary": p.get("summary"),
                    "stances": [],
                    "memories_long": [],
                    "relations_long": [],
                    "entities": [],
                    "alias_map": p.get("alias_map") or r.get("alias_map") or {}
                }
                for item in (p.get("participants") or []):
                    if isinstance(item, dict):
                        nm = item.get("name") or item.get("id") or "?"
                        st = item.get("stance") or []
                        if isinstance(st, str):
                            st=[st]
                        if st:
                            entry["stances"].append({"name": nm, "stance": st})
                for m in (p.get("memories") or []):
                    if LONGTERM_ONLY and (m.get("durability") == "short"):
                        continue
                    who = m.get("speaker_id", "?")
                    txt = (m.get("text") or "").strip()
                    if txt:
                        entry["memories_long"].append({"speaker_id": who, "text": txt})
                for ent in (p.get("entities") or []):
                    en = ent.get("name")
                    et = ent.get("type")
                    als = ent.get("aliases") or []
                    entry["entities"].append({"name": en, "type": et, "aliases": als})
                    for rel in (ent.get("relations") or []):
                        if LONGTERM_ONLY and (rel.get("durability") == "short"):
                            continue
                        subj = rel.get("subject") or en or "?"
                        ...
                        obj = rel.get("object") or en or "?"
                        if pred:
                            entry["relations_long"].append({"subject": subj, "predicate": pred, "object": obj})
                readable_entries.append(entry)
            with open(READABLE_JSON_PATH, "w", encoding="utf-8") as jf:
                json.dump(readable_entries, jf, ensure_ascii=False, indent=2)
            print(f"[INFO] Readable JSON saved to {READABLE_JSON_PATH}")
            # Optional split-per-chunk
            if os.getenv("SPLIT_JSON_DIR", ""):
                outdir = os.getenv("SPLIT_JSON_DIR")
                os.makedirs(outdir, exist_ok=True)
                for entry in readable_entries:
                    idx = entry.get("chunk_index")
                    with open(os.path.join(outdir, f"chunk_{idx:03d}.json"), "w", encoding="utf-8") as cf:
                        json.dump(entry, cf, ensure_ascii=False, indent=2)
                print(f"[INFO] Split JSON written to dir {outdir}")
        except Exception as e:
            print(f"[WARN] Failed to write readable outputs: {e}")
    else:
