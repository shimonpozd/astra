...
async def main():
    ...
    QDRANT_COLLECTION_NAME = os.getenv("QDRANT_COLLECTION_NAME", "chat_facts")
    QDRANT_BATCH_SIZE = int(os.getenv("QDRANT_BATCH_SIZE", "50"))
    QDRANT_PROGRESS_FILE = os.getenv("QDRANT_PROGRESS_FILE", "qdrant_progress.json")
    ...
    # Read progress for Qdrant upsert
    last_upserted_chunk_index = read_qdrant_progress(QDRANT_PROGRESS_FILE)
    print(f"[INFO] Resuming Qdrant upsert from chunk index: {last_upserted_chunk_index + 1}", flush=True)
        ...
        with open(OUTPUT_JSON_PATH, "r", encoding="utf-8") as f:
            ...
            for line in tqdm(f, total=total_lines, desc="Upserting to Qdrant", unit="fact"):
                try:
                    ...
                    chunk_index = fact.get("chunk_index", -1)

                    if chunk_index <= last_upserted_chunk_index:
                        continue # Skip already upserted facts
                    ...
                    if "point_id" in fact and "semantic_summary" in fact: # Check for semantic_summary to embed
                        try:
                            ...
                            fact["embedding"] = embedding # Add embedding to fact for payload
                            qdrant_uploader.add_point(fact["point_id"], embedding, fact)
                            processed_count += 1
                            write_qdrant_progress(QDRANT_PROGRESS_FILE, chunk_index) # Update progress
                        except Exception as e:
                            print(f"[ERROR] Failed to generate embedding for chunk {chunk_index}: {e}", flush=True)
                    else:
