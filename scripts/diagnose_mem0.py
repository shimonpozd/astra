...
def diagnose():
    ...
    collection_name = os.getenv("MEM0_QDRANT_COLLECTION", "astra_memory")
    
    logging.info(f"--- mem0 Diagnostic Script ---")
    ...
    try:
        ...
        # Check for the text index specifically on the 'data' field
        text_index_exists = False
        if collection_info.payload_schema and 'data' in collection_info.payload_schema:
            index_params = collection_info.payload_schema['data']
            if hasattr(index_params, 'data_type') and str(index_params.data_type) == 'text':
                text_index_exists = True

        if text_index_exists:
            logging.info("✅ SUCCESS: A 'text' index with type TEXT exists for the 'data' field. This is correct.")
        else:
            logging.error("❌ FAILURE: A TEXT index for the 'data' field was NOT found. This is the root cause of the problem.")
            logging.error("Please run the `create_mem0_index.py` script to create the correct index.")
    ...
    try:
        ...
        if search_results:
            ...
            for i, point in enumerate(search_results):
                logging.info(f"  - Result {i+1}: {point.payload.get('data')}")
        else:
