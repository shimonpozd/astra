...
# VAD sensitivity threshold. Lower is more sensitive.
VAD_THRESHOLD = 0.2
# Milliseconds of silence to consider the end of a phrase.
MIN_SILENCE_MS = 3000
...
CHANNELS = 1
RATE = 16000
# Silero model expects specific chunk sizes. 512 for 16kHz.
CHUNK_SAMPLES = 512
...
def load_model():
    ...
    try:
        ...
        # get_speech_ts function is what we need
...
def send_to_stt(audio_data: bytes):
    ...
    try:
        # Package as a WAV file in memory
        ...
        # Send as hex string to avoid JSON issues with raw bytes
        response = requests.post(STT_SERVICE_URL, json={"audio_data": wav_bytes.hex()}, timeout=10.0)
...
def vad_pipeline():
    pa = pyaudio.PyAudio()
    stream = pa.open(format=FORMAT, channels=CHANNELS, rate=RATE, input=True, frames_per_buffer=CHUNK_SAMPLES)
    ...
    while state.is_running:
        try:
            ...
            if speech_dict and 'start' in speech_dict:
                ...
                silence_chunks = 0 # Reset silence counter on speech
                voiced_frames.append(chunk_bytes)
            elif is_speaking:
                # Still speaking, but this chunk is silent
                voiced_frames.append(chunk_bytes)
                silence_chunks += 1
                if silence_chunks > silence_timeout_chunks:
                    is_speaking = False
                    logger.info("VAD: Speech end detected due to silence timeout.")
                    threading.Thread(target=send_to_stt, args=(b''.join(voiced_frames),), daemon=True).start()
...
def start_listening():
    if state.is_running: raise HTTPException(status_code=400, detail="Service is already running.")
    if not state.vad_model: raise HTTPException(status_code=503, detail="VAD model not loaded yet.")
...
def stop_listening():
    if not state.is_running: raise HTTPException(status_code=400, detail="Service is not running.")
    state.is_running = False
    if state.audio_thread: state.audio_thread.join(timeout=2.0)
