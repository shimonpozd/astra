...
# ----------------- LLM Extractor -----------------
class LLMExtractor:
    def __init__(self, provider: str, model: str, openai_api_key: Optional[str] = None, ollama_base_url: str = "http://localhost:11434", groq_api_key: Optional[str] = None):
    ...
    def _get_system_prompt(self) -> str:
        return (
            "Ты — профессиональный аналитик прослушки. Диалог двух людей: Шимон и Казах . Твоя задача — собрать информацию, факты и характеристики о них и о их окружении. "
            "Каждый факт — одно короткое, самодостаточное утверждение без воды и пояснений. Только конкретика из текста, без догадок и внешних знаний. Если нет устойчивых фактов — верни пусто."
            "Фокус: характеристики людей, их отношения, биографические факты, устойчивые мнения и привычки, оценки других людей, важные события, характерные выражения/рефрены/клички. Установки и вкусы: «любит/не любит X», «нравится/раздражает Y», отношение к конкретным людям.Игнорируй одноразовые бытовые мелочи типа «еду домой», «пойдём покурим» — это не устойчивые факты."
        ...
        for fact in llm_facts:
            ...
            for i in support_indices_raw:
                try:
                    ...
                    pass # Ignore indices that are not valid integers
