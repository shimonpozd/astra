...
async def get_text_with_window(ref: str, window_size: int = WINDOW_SIZE) -> Optional[Dict[str, Any]]:
    ...
    # Extract texts based on language content
    if focus_data: # Only proceed if we have some focus_data
        ...
        if not containsHebrew(text_from_en_result):
            ...
            logger.warning(f"GET_TEXT_WINDOW: Sefaria returned Hebrew for English request for {ref}. Ignoring for en_text.")
        ...
        if containsHebrew(text_from_he_result):
            ...
            logger.warning(f"GET_TEXT_WINDOW: Sefaria returned non-Hebrew for Hebrew request for {ref}. Ignoring for he_text.")
